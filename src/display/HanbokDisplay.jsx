...
const HanbokDisplay = () => {
    ...
    function checkImageListContainer() {
        if (imageListRef.current) {
            console.log(imageListRef.current.children)
        }
    }
                        ...
                        <div className='flex flex-1 items-center'>
                            <div>
                                <GrFormPrevious size={32}
                                    onClick={() => {console.log(imageListRef)}}
                                    >
                                </GrFormPrevious>
                            </div>
                            ...
                            <div className='bg-slate m-2'>
                                <GrFormNext size={32} onClick={() => {checkImageListContainer()}}></GrFormNext>
                            </div>

                        </div>
                        <div className='mt-4 flex mobile:grid mobile:grid-cols-4 justify-center ' ref={imageListRef}>
                        {imageLength.map((num) => 
                            <img src={IMAGE_PATH + `Store/[${id}]/${num}.jpg`} alt={imageData.display_code} id={num.toString()}
                                className='p-2 hover:bg-slate-200 rounded-lg w-20' 
